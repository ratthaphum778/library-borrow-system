...
def ensure_borrow_schema():
    """
    สร้าง schema borrow + ทำ migration column อัตโนมัติ
    ปลอดภัยกับ DB เก่า (ไม่ต้องลบไฟล์)
    """
    ...
    # ---------------------------
    # 2️⃣ Migration — เพิ่ม column ที่หาย
    # ---------------------------
    ...
    ensure_column("borrow_tx", "default_due_date", "default_due_date TEXT")
    ensure_column("borrow_tx", "status", "status TEXT DEFAULT 'open'")
    ensure_column("borrow_tx", "note", "note TEXT")

    ensure_column("borrow_items", "due_date", "due_date TEXT")
    ensure_column("borrow_items", "return_date", "return_date TEXT")
    ensure_column("borrow_items", "status", "status TEXT DEFAULT 'borrowed'")
    ensure_column("borrow_items", "return_staff_user_id", "return_staff_user_id INTEGER")
...
# ==========================================================
# REPORT FUNCTIONS (SAFE VERSION — กัน error column mismatch)
# ==========================================================

def get_book_status_summary():
    conn = get_connection()

    # เผื่อ books ไม่มี column status
    try:
        df = pd.read_sql_query("""
            SELECT
                COALESCE(status,'unknown') AS สถานะหนังสือ,
                COUNT(*) AS จำนวน
            FROM books
            GROUP BY COALESCE(status,'unknown')
        """, conn)
    except Exception:
        # fallback ถ้าไม่มี status column
        df = pd.DataFrame({
            "สถานะหนังสือ": ["unknown"],
            "จำนวน": [0]
        })
